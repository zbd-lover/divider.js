...
var indexMap = [["before", 0], [0, 0], ["0", 0], ["after", 1], [1, 1], ["1", 1], ["create", 2], ["2", 2], [2, 2], ["3", 3], [3, 3], ["interrupt", 3]];
...
function createSource(processor, discrete) {
  ...
  function tryRunDelay(type) {
    ...
    observeOne(type, tag, fn); // It's possible that one type's some delays exists  at same time, e.g before and create
  ...
  function createDispatch(type) {
    ...
    var sm = creatStateMachine(type, 3, function (sm) {
      observers[2].forEach(function (fn) {
        return sm.hook("create", fn, HOOK_ORDER_MAP['create'].observer);
      });
    ...
    observers[3].forEach(function (fn) {
